...
function collectApiMeta(rootDir: string): ApiMeta {
  ...
  for (const filePath of propFiles) {
    ...
    const compName = idx >= 0 && rel[idx + 1] ? rel[idx + 1] : path.basename(path.dirname(path.dirname(filePath)))
            ...
            if (ts.isObjectLiteralExpression(desc)) {
              for (const dp of desc.properties) {
                ...
                if (key === 'type') {
                  ...
                  if (ts.isAsExpression(initExpr) || ts.isSatisfiesExpression(initExpr)) {
                    ...
                    if (ts.isTypeReferenceNode(typeNode) && typeNode.typeArguments?.length) {
                      ...
                      typeText = checker.typeToString(t, undefined, ts.TypeFormatFlags.NoTruncation)
                    } else {
                      typeText = primitiveFromCtor(initExpr.expression) ?? 'any'
                    }
                  } else {
...
export function amuDocsApiPlugin(): Plugin {
  ...
  const getApiCode = () => `export default ${JSON.stringify(collectApiMeta(rootDir), null, 2)};`
  const getNavCode = () => `export default ${JSON.stringify(collectNavMeta(rootDir), null, 2)};`
  ...
  return {
    ...
    configureServer(server) {
      ...
      const watchFiles = walk(componentsRoot, (p) => p.endsWith(path.join('src', 'props.ts')))
